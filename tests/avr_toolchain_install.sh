...
if [[ "`uname`" == "Darwin" ]]; then
	if [[ ! -e $MYDIR/avr8-gnu-toolchain-darwin_x86_64 ]]; then
		cd $MYDIR/
		curl -o avr8-gnu-toolchain-osx.tar.gz 'http://distribute.atmel.no/tools/opensource/Atmel-AVR-GNU-Toolchain/3.5.4/avr8-gnu-toolchain-osx-3.5.4.468-darwin.any.x86_64.tar.gz'
...
if [[ "`uname`" == "Linux" ]]; then
	if [[ ! -e $MYDIR/avr8-gnu-toolchain-linux_x86_64 ]]; then
		cd $MYDIR/
		curl -o avr8-gnu-toolchain-linux.tar.gz 'http://www.atmel.com/images/avr8-gnu-toolchain-3.5.4.1709-linux.any.x86_64.tar.gz'
