...
FatFile* FatFile::m_cwd = nullptr;
// Callback function for date/time.
void (*FatFile::m_dateTime)(uint16_t* date, uint16_t* time) = nullptr;
...
bool FatFile::contiguousRange(uint32_t* bgnBlock, uint32_t* endBlock) {
	...
	for (uint32_t c = m_firstCluster;; c++) {
		...
		if (fg == 0 || next != (c + 1)) {
			...
			*endBlock = m_vol->clusterStartBlock(c) + m_vol->blocksPerCluster() - 1;
			return true;
		}
	}
...
int16_t FatFile::read(void* buf, size_t nbyte) {
	...
	if (isFile()) {
		...
		uint16_t tmp16 = 32 * m_vol->m_rootDirEntryCount - static_cast<uint16_t>(m_curPosition);
		if (nbyte > tmp16) {
			nbyte = tmp16;
		}
	}
	...
	while (toRead) {
		...
		const auto lock_set(FatVolume::os_lock());
...
bool FatFile::sync() {
	...
	const auto res(m_vol->cacheSync());
	if (! res) {
		DBG_FAIL_MACRO;
	}
	FatVolume::os_unlock(lock_set);
...
bool FatFile::timestamp(uint8_t flags, uint16_t year, uint8_t month, uint8_t day, uint8_t hour, uint8_t minute, uint8_t second) {
	uint16_t dirDate;
	uint16_t dirTime;
	dir_t* dir;

	if (! isFile() || year < 1980 || year > 2107 || month < 1 || month > 12 || day < 1 || day > 31 || hour > 23 || minute > 59
			|| second > 59) {
		DBG_FAIL_MACRO;
		goto fail;
	}
...
int16_t FatFile::write(const void* buf, size_t nbyte) {
	...
	while (nToWrite) {
		...
		if (blockOffset != 0 || nToWrite < 512) {
			...
			__builtin_avr_cli();
			if (m_vol->cacheBlockNumber() <= block && block < (m_vol->cacheBlockNumber() + nBlock)) {
