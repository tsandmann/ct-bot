...
void bot_goto_behaviour(Behaviour_t * data) {
	...
	if (mot_goto_l > 0) {
		...
		if (((speedWishLeft < 0) && (direction.left == DIRECTION_FORWARD))
				|| ((speedWishLeft > 0)
	...
	if (mot_goto_r > 0) {
		...
		// Wenn neue Richtung ungleich alter Richtung
		if (((speedWishRight < 0) && (direction.right == DIRECTION_FORWARD))
				|| ((speedWishRight > 0) && (direction.right == DIRECTION_BACKWARD)))
