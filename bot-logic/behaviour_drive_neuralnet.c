...
static uint8_t teachout0 = 0;	/**< Belegungen der Output-Neuronen beim Speichern der Patterns */
...
static uint8_t goal_reached(void) {
	...
	if ((sensLineL > GROUND_GOAL_DEF - 5 && sensLineL < GROUND_GOAL_DEF + 5) || (sensLineR > GROUND_GOAL_DEF - 5 && sensLineR < GROUND_GOAL_DEF + 5)) {
		LOG_DEBUG("Zielfeld erreicht->Endbedingung");
...
/**
 *  dient zur Ausrichtung an der Wand nach Geradeausfahren
 */
...
void bot_drive_neuralnet_behaviour(Behaviour_t * data) {
	...
	case BOT_CHECK_WALL_AFTER_FORWARD:	// kommt hierher nach Geradeausfahren, um sich wieder parallel zur Wand auszurichten
 ...
 * \param *caller	  Verhaltensdatensatz
 * \param out0        Wert fuer die Belegung des Output-Neurons 0 fuer die Wunschaktion
 * \param out1        Wert fuer die Belegung des Output-Neurons 1 fuer die Wunschaktion
 * \param wishaction  nach sector-Check soll der Bot diese Aktion ausfuehren, was den Werten in out0 out1 entspricht
 */
void bot_check_sectors_savepattern(Behaviour_t * caller, uint8_t out0, uint8_t out1, uint8_t wishaction) {
	if (no_of_pairs >= NO_PAIRS) {
		...
		switch_to_behaviour(caller, bot_check_sector_behaviour, BEHAVIOUR_OVERRIDE);
		check_sector_state = 0;
		// gewuenschte Outputs mit den sector-Werten zusammen abspeichern
		teachout0 = out0;
		teachout1 = out1;
		bot_wish_action = wishaction;
	}
