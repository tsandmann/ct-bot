...
//				float dPos=0;					/*!< aktuelle Positionsaenderung  */
	...
	if (timer_get_ms_since(olds_speed,oldms_speed)>250) {	// sollte genau 1x pro Sekunde zutreffen
		...
		v_enc_left=  (((sensEncL - lastEncL1) * WHEEL_PERIMETER) / ENCODER_MARKS)*0.5;
		v_enc_right= (((sensEncR - lastEncR1) * WHEEL_PERIMETER) / ENCODER_MARKS)*0.5;
		...
		lastEncR1= sensEncR;
			...
			v_mou_right=lastHead/360*2*M_PI*right_radius*0.5;
			v_mou_left=lastHead/360*2*M_PI*left_radius*0.5;
			lastDistance=0;
			lastHead=0;
			old_x=x_mou;
			old_y=y_mou;
